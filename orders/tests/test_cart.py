...
class CartTests(APITestCase):
    ...
    def test_update_cart_item_quantity(self):
        ...
        data = {"store_item_id": self.store_item.id, "quantity": 2}
        response = self.client.post(url, data, format="json")
        cart_item_id = response.data["items"][0]["store_item_id"]
    ...
    def test_remove_item_from_cart(self):
        ...
        url = reverse("mycart-remove-item")
        response = self.client.delete(url, {"cart_item_id": cart_item_id}, format="json")
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(self.client.get(reverse("mycart-detail")).data["items"]), 0)

        
    # def test_add_more_than_stock(self):
    #     url = reverse("mycart-add-to-cart")
    #     data = {"store_item_id": self.store_item.id, "quantity": 999}
    #     response = self.client.post(url, data, format="json")

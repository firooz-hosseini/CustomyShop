...
class StoreItemTests(APITestCase):
    def setUp(self):
        self.seller_user = User.objects.create_user(
            email="seller@example.com", password="pass123", role="seller"
        )
        self.other_user = User.objects.create_user(
            email="other@example.com", password="pass123", role="seller"
        )

        self.store = Store.objects.create(name="Owner Store", seller=self.seller_user)

        self.category = Category.objects.create(name="Test Category", description="Category desc")
